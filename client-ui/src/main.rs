//! ModelLock 买家端桌面 UI（demo）
//! 页面：我的模型 / 信任作者 / 设置；可爱浅色风。
...
const PINK_SOFT: egui::Color32 = egui::Color32::from_rgb(255, 235, 242);
...
struct App {
    page: Page,
    models: Vec<core::auth::AcceptedLicense>,
    trusted: bool,
...
impl App {
    fn new() -> Self {
        let models = core::list_models().unwrap_or_default();
        let trusted = core::is_author_trusted().unwrap_or(false);
        Self {
            page: Page::Library,
            models,
            trusted,
    ...
    fn refresh_models(&mut self) {
        self.models = core::list_models().unwrap_or_default();
    }

    fn do_export_vreq(&mut self) {
        match rfd::FileDialog::new()
            .set_file_name("授权请求.vreq")
            .save_file()
        {
            Some(path) => match core::init_device(&path) {
                Ok(info) => {
                    self.log(format!("已导出授权请求 key_id={}", &info.key_id[..8.min(info.key_id.len())]));
                }
    ...
    fn do_trust_author(&mut self) {
        match rfd::FileDialog::new()
            ...
            .pick_file()
        {
            Some(path) => match core::trust_author_file(&path) {
                Ok(key_id) => {
                    self.trusted = true;
                    self.author_key_id = key_id.clone();
                    self.log(format!("已信任作者密钥 {}", &key_id[..8.min(key_id.len())]));
                }
    ...
    fn do_mount(&mut self) {
        ...
        match core::verify_and_accept(&path_p, code.as_deref()) {
            Ok(entry) => {
                let cfg = MountConfig {
                    kill_vts_on_unmount: self.kill_vts,
                    ..Default::default()
                };
                match core::mount_model(&path_p, None, &cfg) {
                    Ok(handle) => {
                        self.mounted_model = Some(entry.model_id.clone());
    ...
    fn do_unmount(&mut self) {
        if let Some(mut handle) = self.mount_handle.take() {
            handle.stop();
            self.log("正在卸载…（VTS 保持运行）");
        }
        self.mounted_model = None;
    }

    fn do_mount_existing(&mut self, model_id: &str) {
        let entry = self.models.iter().find(|m| m.model_id == model_id).cloned();
        ...
        match core::mount_model(&path, None, &cfg) {
            Ok(handle) => {
                self.mounted_model = Some(entry.model_id.clone());
                self.mount_handle = Some(handle);
                self.log(format!("已挂载 {}", entry.model_id));
        ...
        egui::SidePanel::left("nav").show(ctx, |ui| {
            ...
            egui::ScrollArea::vertical().max_height(220.0).show(ui, |ui| {
                for m in self.messages.iter().rev().take(30) {
                    ui.label(egui::RichText::new(m).size(11.0).color(egui::Color32::from_rgb(120, 110, 130)));
                }
            });
        });
...
impl App {
    fn ui_library(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(path) = &self.pending_vkit {
            ui.add_space(8.0);
            ui.group(|ui| {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for m in self.models.clone() {
                ui.group(|ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.button("移除").clicked() {
                                let _ = core::remove_model(&m.model_id);
                                self.refresh_models();
                            }
                            if ui.button("挂载").clicked() {
                    ...
                    ui.label(egui::RichText::new(&m.vkit_path).size(11.0).color(egui::Color32::from_rgb(140, 130, 150)));
                });
                ui.add_space(6.0);
            }
        });
